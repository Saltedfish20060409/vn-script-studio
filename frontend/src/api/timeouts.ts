 ...
 *
 * 还有一项后端加时：**长提示词的预填充**（`llm_budget.PREFILL_*`，默认最多 +60s）。
 * 上下文预算放大之后（`agent_context.DEFAULT_CONTEXT_MAX_CHARS`），非流式请求的
 * read timeout 要覆盖"预填充 + 生成"，所以这里按 `基础×系数 + 预填充上限` 对齐。
 * `timeouts.test.ts` 直接读后端的 `PREFILL_MAX_BONUS`，后端改数而不改这里会红。
 *
