...
function Wait-Port([int]$Port, [int]$TimeoutSec = 45) {
  ...
  while ((Get-Date) -lt $deadline) {
    ...
    # Fallback: TCP connect (Get-NetTCPConnection sometimes lags on Windows)
    try {
...
function Wait-IfNeeded([int]$Code = 0) {
  # When launched from dev.bat (-KeepOpen), bat will pause; just exit with code.
  exit $Code
}
...
try {
  ...
  # Strip UTF-8 BOM from .env (Windows editors often add it; breaks DATABASE_URL load)
  ...
  if (Test-Port 8000) {
    ...
    if ($needReplace) {
      ...
      # also stop any leftover uvicorn spawn workers
      ...
      # same venv — still verify schema has kind (old workers may linger)
      $hasKind = $false
      try {
