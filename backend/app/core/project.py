...
def extract_map_from_script(project: VnProject) -> Dict[str, Any]:
    ...
    for image in scenes:
        ...
        if loc is None:
            ...
            existing_by_tag[key] = loc
        if loc.id not in seen_in_pass:
            seen_in_pass.add(loc.id)
            order_ids.append(loc.id)

    old_links = project.locationLinks or []
    link_keys = {f"{l.fromId}->{l.toId}:{l.relation}" for l in old_links}
    ...
    for i in range(len(order_ids) - 1):
        ...
        if from_id == to_id:
            continue
        key = f"{from_id}->{to_id}:leads_to"
        if key in link_keys:
            continue
        location_links.append(new_location_link(from_id, to_id, "leads_to"))
        link_keys.add(key)
        link_count += 1
...
_SCENE_PRESET_TABLE = [
    ...
    (re.compile(r"bridge|桥|通道|tunnel"), "bridge"),
