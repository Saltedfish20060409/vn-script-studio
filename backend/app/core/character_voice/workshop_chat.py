...
def _persona_block(c: Character) -> str:
    parts = [
    ...
    *,
    ...
    if mode == "duo":
        ...
        system = (
            ...
            "生成他们接下来的交锋对白。\n"
            "输出 JSON：{ lines:[{speakerId, speakerName, text}, ...] }，2～4 句交替。\n"
            "严格服从各自思维包与正例感觉；禁止设定说明书；像能演的 VN 对白。"
        )
        user = "\n".join(
            [
                "## 角色 A（焦点）",
                _persona_block(focus),
                "## 角色 B（搭档）",
                _persona_block(partner),
                "## 近期对白",
                hist_txt or "（无）",
                f"## 用户出题/旁白\n{msg}",
    ...
    if mode == "duo":
        ...
        if isinstance(raw_lines, list):
            for ln in raw_lines:
                ...
                lines_out.append(
                    {"speakerId": sid, "speakerName": sname or sid, "text": text}
                )
        if not lines_out:
