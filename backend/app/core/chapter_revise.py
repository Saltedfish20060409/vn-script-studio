...
改前：带你熟悉一下这里。这里是厨房 / 那边是吃饭的地方……
改后：你领她走过客厅，没有解说。她自己停了一下，看向窗户。

改前：明白了。为了达成「新生活」的要求…… / 也不用当成任务来完成。
改后：她轻轻点头，把这句话留在空气里，没有收成任务清单。

改前：为什么要分开？盐是现在撒吗？原理就是……
改后：旁白两句带过工序；留入口感受（烫、香、像家）。
...
def _hard_fail_snippets(text: str) -> List[str]:
    ...
    if re.search(
        r"这里是厨房|这里是我的工作区|带你熟悉一下这里|熟悉一下这个空间|那边是吃饭的地方|冰箱里有食材.?饿了|带你熟悉一下|我来给你介绍一下这里",
        text,
    ):
...
def _find_menu_insert_pos(draft: str) -> int:
    ...
    for pat in (r"起名", r"叫什么", r"给你取", r"名字", r"编号", r"VII"):
