...
def _notes(
    *, long_paragraph_chars: int, coverage: Dict[str, Any]
    ...
    return [
        ...
        "注音**渲染**已在 `core/ruby_render.py` 落地：Markdown 输出 W3C 的 `<ruby>/<rt>`"
        "（含 `<rp>` 回退），Word 稿与 .rpy 输出 `<rp>` 回退的纯文本形态"
        "（`漢字（かんじ）`）；**Word 原生注音（`w:ruby`）仍未做**——"
        "python-docx 不支持，且生成的文件能否被 Word 正常打开在这里无法验证。",
        "已知未覆盖：拟声词表不完整；对白「信息量」「节奏好坏」这类语义判断不在本模块范围内。",
