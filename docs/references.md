...
| 《「讀」與「玩」的邊界消融：文字小說改編為視覺小說的實踐與敘事重構分析》 | **未核实**（刊名/年份/期号待确认） | 小说 → VN 改编实践与叙事重构 | 暂不落库；能核实时再补（预期落点是"改编检查表"） |
...
- [ ] 第二轮（已核实的补充书目，待做）：*Branching Story, Unlocked Dialogue*（分支与对话设计实务）、
      『ノベルゲームのシナリオ作成技法』（AVG 剧本作法）、〈視点が担うメッセージ〉（视角承载信息）、
      『キャラクター小説の作り方』（角色先于情节）、ビジュアルノベル先行研究サーベイ、
      JSET 2024「正解/部分正解/不正解」的分支结果分层、中文 VN 学位论文两篇。
      预期落点（各配测试）：**选项后果分层**检查、**改编检查表**（小说 → VN）、
      "视角切换是否有标记"检查
