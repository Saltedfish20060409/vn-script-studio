...
（它会让我们在导出时把正文吞进台词），所有表记规则都只报 warn/info，并附依据，
由作者定夺。

### 视觉小说 / 轻小说实务（参考层）

这一组不是"规范"，是**实务与研究**。用它们的方式与论文那条一样：能变成**可测读数**的才落进代码，
审美判断只当解读框架。逐条注明我核实到了什么程度（2026-09）。

| 文献 | 核实情况 | 管什么 | 落在哪 |
|---|---|---|---|
| 《轻浅的美学：论日本轻小说的文体特征与审美价值》（硕士学位论文，[万方](https://apps.wanfangdata.com.cn/thesis/article:Y3040105)） | 查到学位论文记录 | 轻小说文体特征：**短句、对白占比高、第一人称、拟声/拟态词、注音（ルビ）** | `novel_craft`：`sentence_stats`（句长 P50/P90、短句比/长句比）、`person_stats`（地の文第一/第三人称占比）、既有的对白占比 / 拟声密度 / 注音密度；汇总成 `styleProfile.readings` |
| 《ライトノベル表現論：会話・創造・遊びのディスコースの考察》（[国語研文献库](https://bibdb.ninjal.ac.jp/bunken/ja/article/300000227213)） | 查到书目记录 | 轻小说以**会话**为文体中心 → "对白驱动 / 叙述驱动"是可以描述的分档 | 同上：`styleProfile` 的「对白 / 叙述」读数（分档描述**不产出建议**） |
| 《日本轻小说模式的演变及特征》（出版发行研究 2017(08)，[万方](https://d.wanfangdata.com.cn/periodical/cbfxyj201708024)） | 查到期刊记录 | 连载/文库本形态决定节奏：一章一个起落、章末留钩子 | `styleProfile` 的「连载节奏」读数（平均章字数 + 章末钩子中位）；连载工作台与投稿自检的说明文字 |
| 《视觉小说叙事逻辑的根源与变迁》（新闻传播 2022(15)，[CNKI](https://wap-cnki-net-443.webvpn.imac.edu.cn/touch/web/Journal/Article/YWCB202215016.html)） | 查到期刊记录 | 视觉小说的谱系：文字小说 + 选择支 → 声音小说 / kinetic（**无选项也是合法形态**） | 体裁用词与帮助页的口径（VN/LN 两条路径都成立，不把"必须有选项"当默认）；**不改代码** |
| 冲方丁《冲方丁のライトノベルの書き方講座》（[CiNii](https://ci.nii.ac.jp/ncid/BB11832389)） | 查到书目记录 | 写作过程与结构（开场、推进、收束）的行业经验 | **只作参考**：作为既有工艺卡/提示词口径的旁证，**不新增规则**（技法书是经验不是规范） |
| 《すごいライトノベルが書ける本：これで万全!創作テクニック》（[CiNii](https://ci.nii.ac.jp/ncid/BB15868752)、[NDL](https://ndlsearch.ndl.go.jp/books/R100000136-I1130282272157922048)） | 查到书目记录 | 技法条目（开场、钩子、人物魅力） | 同上：工艺卡素材来源，不写成硬规则 |
| 《「讀」與「玩」的邊界消融：文字小說改編為視覺小說的實踐與敘事重構分析》 | **不收录**（出处查不到：刊名/年份/期号均无法核对，2026-09 作者决定不再补） | 小说 → VN 改编实践与叙事重构 | 不落库。它要说的那件事（改编要先切场、无选项也是合法形态）已由本表**已核实**的条目覆盖，并落在 `core/adaptation_checklist.py` |
| 《叙事、创作、接受：构建视觉小说美学的三个维度》（中国现代文学论丛 2025(1)） | **不收录**（同上，出处无法核对） | VN 美学的三个维度（叙事/创作/接受） | 不落库。我们的三层（结构量 / 工具与写作 / 试玩遥测与盲测）与它同类，但没有可执行的新结论；**没有依据的引用不加** |
| [*Branching Story, Unlocked Dialogue: Designing and Writing Visual Novels*](https://books.google.com.sg/books?id=FdKTEAAAQBAJ)（CRC Press） | 查到书目记录 | 分支与对话设计的**实务**：选项要通向不同后果、路线要有各自的展开 | `choice_poetics.analyze_consequence_tiers`：给每个选项判**后果形态**（继续往下走 / 与同菜单别的选项汇合 / 只走到很短的结局 / 看不出），并把"分量悬殊"报成一条可执行建议（`consequence_imbalance`） |
| JSET 2024《正解・部分的正解・不正解に即した結果のストーリー展開を決める》（[J-STAGE](https://www.jstage.jst.go.jp/article/jsetstudy/2024/3/2024_JSET2024-3-A9/_pdf/-char/ja)） | 读到第 4 条设计步骤原文 | 按**正解 / 部分正解 / 不正解**分别决定结果的展开 | 同上：三种形态在结构上对应"继续 / 汇合（走了弯路又回来）/ 短结局"；**不新增规则**（该不该有坏结局是设计选择） |
| ビジュアルノベル先行研究サーベイ（[デジタルゲーム学研究](https://www.jstage.jst.go.jp/article/digrajproc/15/0/15_121/_pdf/-char/ja)） | 读到摘要段落 | VN 的表达力来自"用选项操作故事 / 坏结局 / 别的路线" | 改编检查表与 `branch_recommendations` 的 **flow 读数**（菜单数 / label 数 / 声明结局 vs 可达结局）；**不把"没有分支"当缺陷** |
| 『ノベルゲームのシナリオ作成技法』（[CiNii](https://ci.nii.ac.jp/ba/89448326)） | 查到书目记录 | AVG 剧本实务：**场景切分**、演出与选项位置 | 改编检查表的 `no_scene_markers` 项（哪几章没有分场标记 → 改编时先切场） |
| 『キャラクター小説の作り方』（大塚英志，[CiNii](https://ci.nii.ac.jp/ncid/BB13987353)） | 查到书目记录 | **角色先于情节** | 改编检查表的 `character_material_thin` 项（出场角色有没有可演的声线/语料/思维卡） |
| 《文字冒险游戏设计中的沉浸式体验研究与实践》（[万方学位论文](https://wf.pub/thesis/article:D02140210)）、[《剧情交互式游戏的叙事研究》](https://s.wanfangdata.com.cn/paper?q=%E4%BD%9C%E8%80%85%3A%22%E5%90%B4%E5%BD%A6%E5%A6%8D%22) | 查到学位论文记录 | VN 的沉浸感来自"操作故事 + 对白与演出推进"；交互叙事的结构不同于纯文本 | `core/adaptation_checklist.py`：**改编检查表**（一屏文字 / 对白占比过低 / 分场缺失 / 选项与结局 / 角色素材），逐条带 why 与 action |
| 〈視点が担うメッセージ：『ひぐらしのなく頃に』に見るノベルゲームの物語構成法〉（[NDL](https://ndlsearch.ndl.go.jp/books/R000000004-I026804221)） | 查到书目记录 | **视角如何承载信息**（同一事件换视角看） | **只作参考**：逐段"是否换了视角"在结构上判不准——代词与角色名无法区分"叙述者提到别人"与"换了视角"（第一人称叙述里满篇都是"他/她"）。我们保留**章级** `pov_shift`（偏离全书主导人称）+ 体检面板的人称读数，不新增易误报的规则 |

**这一组的纪律（写进代码注释里了）**：`styleProfile` 只给**读数**与本书内的相对位置，
读数带 `kind="reading"` 与 `basis`，**不产出"应该改成什么样"的建议**——文体偏好是作者的。
要提建议必须另找依据（例如表记类国标），这与"不要用模型给文学打分"（Art or Artifice?）是同一条纪律。
...
- [x] W3C Ruby 剩余（Word 原生注音）：`core/docx_ruby.py` —— **投稿稿默认写 `w:ruby`**
      （按 ECMA-376 的 `CT_Ruby` 组装：`rubyPr`/`rt`/`rubyBase`，`rt` 与 `rubyBase` 里装 run），
      编辑用 Word 打开时注音是真的注音；`SubmissionOptions.native_ruby` 可关掉回退成
      `漢字（かんじ）`。**能验证什么、不能验证什么都写在模块文档里**：结构（元素/顺序/属性）
      与"生成的文件仍能被 python-docx 解析"由 15 项测试钉住；Word 客户端的观感仍无法在这里验证，
      所以字号是参数（默认基准 10pt / 注音 5pt / 抬升 9pt）。阅读导出（`project_to_docx`）仍走 rp：
      原生注音的基准词只在 `w:rubyBase` 里，简单取文本的工具会漏掉，那条导出的用途是"读出来/导回去"。
      **顺带修掉一处会丢字的缺陷**：`file_text` 的 docx 导入过去用 `paragraph.text`
      （只拼直接 run），一份带注音的投稿稿导回工作台会整片丢字——现在按文档顺序走一遍，
...
- [x] 视觉小说 / 轻小说实务（参考层，见第一节末表）：`novel_craft` 新增**文体剖面**——
      `sentence_stats`（句长 P50/P90、短句/长句比）与 `person_stats`（**只统计地の文**的
      第一/第三人称占比），加上既有的对白占比、拟声密度、注音密度、章末钩子，汇总成
      `styleProfile.readings`（每条带 `basis`，**只给读数不给建议**）+ 13 项测试；
      界面在「项目 → 稿件体检」显示读数块与"句长中位"列。
      依据：《轻浅的美学》（短句/对白/一人称/拟声/注音）、《ライトノベル表現論》（会话中心）、
      《日本轻小说模式的演变及特征》（连载节奏）、《视觉小说叙事逻辑的根源与变迁》（体裁口径：
      kinetic 也是合法形态，不把"必须有选项"当默认）
- [x] 第二轮（已核实的补充书目）：**选项后果分层**（`choice_poetics.analyze_consequence_tiers`
      + `consequence_imbalance` 建议 + `branch_recommendations` 接线 + 8 项测试）与
      **改编检查表**（`core/adaptation_checklist.py`，六个结构项 + 逐章读数 + 角色素材，
      `parts=adapt` 路由 + 界面「项目 → 稿件体检」一节 + 13 项测试）。
      依据：*Branching Story, Unlocked Dialogue*、JSET 2024（正解/部分正解/不正解）、
      ビジュアルノベル先行研究サーベイ、『ノベルゲームのシナリオ作成技法』、
      『キャラクター小説の作り方』、《文字冒险游戏设计中的沉浸式体验研究与实践》、
      《剧情交互式游戏的叙事研究》
- [x] 第二轮里**判定只作参考**的一条：〈視点が担うメッセージ〉（ひぐらし视角论）——
      逐段视角切换在结构上判不准（代词/角色名分不清"提到别人"与"换视角"），
      保留章级 `pov_shift` 与体检面板的人称读数，理由写在上表
- [x] 两条**不收录**（2026-09 作者决定，不再列为待办）：
      《「讀」與「玩」的邊界消融：文字小說改編為視覺小說的實踐與敘事重構分析》、
      《叙事、创作、接受：构建视觉小说美学的三个维度》。理由有两条：
      ①出处查不到（刊名/年份/期号均无法核对），而本仓库的规矩是"依据要能核对"；
      ②它们要说的那件事（小说→VN 改编要先切场、要能给角色可演的东西、无选项也是合法形态）
      已由表中**已核实**的条目覆盖并在 `core/adaptation_checklist.py` 落地，
      再加两行没有可执行新结论的引用，只会让依据表变长而不变准

---

## 五、软约束 vs 硬门（实现纪律）

对模型来说，skills / 长提示词 /「请遵守」一律是**软约束**。本仓库的纪律是：

1. **能证伪的进硬门**：国标 lint、确定性体检、`harness_editor_pass` / pipeline revise
   「全过则跳过 LLM」、标记建议在无作者要求且体检干净时不调模型、暴露率/记忆探针进 CI。
2. **编排决定「能不能看见」**：`_SECTION_ORDER`、超预算整块让位并写明缺失（半硬）。
3. **风格与文学只给人审**：`styleProfile` 只出读数；界面 basis **不挂论文名当规范**；
   文献索引留在本文，不冒充作者必遵。
4. **作者硬规则分层**：标记词挑出硬/软；**可证伪子集**（目前：叙述人称）进
   `check_executable_hard_rules` → 稿件体检；其余仍进提示词末尾（承认是软的）。
5. **B 层语义矛盾不靠加长提示词**：长程量具区分 A/B 层；确定性检测器不得假装召回 B 层。
6. **写后廉价闸**：续写/改写产出正文后默认跑 `write_gate`（lint + 说明书硬伤）；
   失败标警告，不挡落地；并给出可送进「标记批改」的失败段 quote（不全章重写）。
7. **本场必带**：`writingGoals.mustBring` 进上下文头部且不进整块让位表；记忆探针
   `must_bring` 维可测「有没有进窗」。
